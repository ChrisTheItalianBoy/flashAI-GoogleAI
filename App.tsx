...
const STRIPE_PAYMENT_LINK = 'https://buy.stripe.com/9B67sL8Vg5Aq92Wdxo38401';
...
const AppContent: React.FC = () => {
  ...
  // Payment page status handled via redirect, fallback if state persists
  if (status === AppStatus.PAYMENT) {
      handlePaymentStart();
      return <div className="flex items-center justify-center min-h-screen">Redirecting to checkout...</div>;
  }
  ...
  return (
    <div className="min-h-screen bg-grid dark:bg-slate-900 dark:bg-none text-slate-900 dark:text-white font-sans selection:bg-indigo-100 transition-colors duration-300">
      <header className="bg-white/90 dark:bg-slate-900/90 backdrop-blur border-b border-slate-200 dark:border-slate-800 sticky top-0 z-50">
        <div className="max-w-5xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2 cursor-pointer" onClick={() => user ? setStatus(AppStatus.IDLE) : setStatus(AppStatus.LANDING)}>
             ...
             {user && (
               <div className="flex items-center gap-4">
                 ...
                 <div className="relative" ref={profileMenuRef}>
                   ...
                   {isProfileOpen && (
                     <div className="absolute right-0 top-full mt-2 w-64 bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-slate-100 dark:border-slate-700 py-2 z-50 animate-fade-in origin-top-right">
                       ...
                       {isFreeTier && (
                           <button 
                              ...
                              className="w-full text-left px-4 py-2.5 text-sm text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 flex items-center gap-2 font-bold"
                           >
